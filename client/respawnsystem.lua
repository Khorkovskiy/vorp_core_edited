    ...
    if not carried then
        if not Done then
            local label = CreateVarString(10, 'LITERAL_STRING',
                T.RespawnIn ..
                TimeToRespawn .. T.SecondsMove .. T.message .. "~m~(" .. TimeToRespawn2 .. " seconds, on place)")
            local label2 = CreateVarString(10, 'LITERAL_STRING',
                T.RespawnIn ..
                TimeToRespawn .. T.SecondsMove .. T.message .. "~m~(" .. TimeToRespawn2 .. " seconds, on place)")
            return label and label2
        else
            local label = CreateVarString(10, 'LITERAL_STRING', T.message2)
            local label2 = CreateVarString(10, 'LITERAL_STRING', T.messageON)
            return label and label2
        end
    else
        local label = CreateVarString(10, 'LITERAL_STRING', T.YouAreCarried)
        local label2 = CreateVarString(10, 'LITERAL_STRING', T.YouAreCarried)
        return label and label2
    end
end
...
local function RespawnTimer2()
    TimeToRespawn2 = Config.RespawnTime2
    CreateThread(function() -- asyncronous
        while true do
            Wait(1000)
            TimeToRespawn2 = TimeToRespawn2 - 1
            if TimeToRespawn2 < 0 and setDead then
                TimeToRespawn2 = 0
                break
            end
...
function CoreAction.Player.RespawnPlayer(onplace)
    ...
    if onplace then
        ...
        if coords then
            ...
            TriggerServerEvent("vorpcharacter:getPlayerSkin")
            TriggerServerEvent("checkRespawnOnPlace2")
        else
            print('Error: No hospital locations found')
        end
    end
end
...
CreateThread(function()
    Wait(1000)
    local str = T.prompt2
    local keyPress = Config.RespawnKey2
    prompt2 = PromptRegisterBegin()
    PromptSetControlAction(prompt2, keyPress)
    str = CreateVarString(10, 'LITERAL_STRING', str)
    PromptSetText(prompt2, str)
    PromptSetEnabled(prompt2, 1)
    PromptSetVisible(prompt2, 1)
    PromptSetHoldMode(prompt2, Config.RespawnKeyTime2)
    PromptSetGroup(prompt2, prompts)
...
AddEventHandler("vorp_core:Client:AddTimeToRespawn", function(time)
    ...
    elseif TimeToRespawn <= 1 then
        RespawnTimer()
    end
    if TimeToRespawn2 >= 1 then
        TimeToRespawn2 = TimeToRespawn2 + time
    elseif TimeToRespawn2 <= 1 then
        RespawnTimer2()
    end
end)
                ...
                if not IsEntityAttachedToAnyPed(PlayerPedId()) then
                    ...
                    if UiPromptHasHoldModeCompleted(prompt) then
                        if Config.CanRespawn() then
                            ...
                            Wait(3000)
                            CoreAction.Player.RespawnPlayer(false)  -- Set `false` for hospital respawn
                            PressKey      = true
                            carried       = false
                            Done          = false
                            TimeToRespawn = Config.RespawnTime
                    ...
                    if PromptHasHoldModeCompleted(prompt2) then
                        DoScreenFadeOut(3000)
                        Wait(3000)
                        CoreAction.Player.RespawnPlayer(true)  -- Set `true` for on-place respawn
                        PressKey       = true
                        carried        = false
                        Done           = false
                        TimeToRespawn2 = Config.RespawnTime2
                    ...
                    if TimeToRespawn2 >= 1 and setDead then
                        ProcessCamControls()
                        Done = false
